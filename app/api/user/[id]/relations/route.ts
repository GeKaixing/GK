...
export async function GET(
  ...
  // 当前用户已关注列表（用于 isFollowing）；游客为空集合，isFollowing 全 false
  const myFollowing = user
  ...
  try {
    ...
    if (type === "recommended") {
      ...
      if (!user) {
        ...
        const generic: RecommendedUser[] = genericUsers.map((u) => ({
          id: u.id,
          name: u.name || "用户",
          handle: u.userid,
          avatar: u.avatar,
        ...
        return NextResponse.json({ success: true, users: generic });
      }
    ...
    // 当前用户已关注列表（用于 isFollowing）——已在函数开头按需计算
    const myFollowingIds = new Set(myFollowing.map((f) => f.followingId));
